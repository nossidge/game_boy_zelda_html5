...
This file is being edited directly while I'm in the process of early
prototyping.

If grunt is run, this file will be overwritten by the individual .js files.
...
App.beforeSetup = function() {
  ...
  // Make sure the Tilemaps are loaded before the App starts.
  if (!TILED.allLoaded()) {
    setTimeout(App.beforeSetup, 50);
    return;
  };
...
var Actor = Box.extend({
  ...
  move: function() {
    ...
    // TODO: Also smooth here?
    var pixelZoom = 1;
    this.x = Math.round(this.x / pixelZoom) * pixelZoom;
    this.y = Math.round(this.y / pixelZoom) * pixelZoom;
   ...
   * SpriteMap or if the player does not have a spriteActor object set.
   *
